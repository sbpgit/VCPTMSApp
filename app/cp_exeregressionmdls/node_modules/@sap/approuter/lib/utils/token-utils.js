...
function loadClientCredentialsToken(app, correlationId, credentials, serviceName, cb) {
  ...
  xssec.requests.requestClientCredentialsToken(null, config, null, null, (err, accessToken, tokenInfo) => {
    if (err) {
      tracer.error('Error getting client_credentials token for ' + serviceName + ' error ' + err.message);
      return cb ? cb(err) : null;
    }
    if (!tokenInfo.access_token || !tokenInfo.expires_in) {
      tracer.error('Bad response from UAA when getting client credentials token for ' + serviceName + ' - not all fields are present');
      return cb ? cb(new Error('Bad response from UAA when getting client credentials token for ' + serviceName + ' - not all fields are present')) : null;
    }
