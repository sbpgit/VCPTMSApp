...
function generateCustomAction(basePath, actionConfig, fs) {
    (0, validate_1.validateVersion)(actionConfig.ui5Version);
    if (!fs) {
    ...
    if (config.settings.eventHandler === true) {
        const controllerPath = (0, path_1.join)(config.path, `${config.name}.js`);
        if (!fs.exists(controllerPath)) {
            fs.copyTpl((0, path_1.join)(root, 'common/EventHandler.js'), controllerPath, config);
        }
    ...
    Object.assign(actions, JSON.parse((0, ejs_1.render)(fs.read((0, path_1.join)(root, `action/manifest.action.json`)), config)));
