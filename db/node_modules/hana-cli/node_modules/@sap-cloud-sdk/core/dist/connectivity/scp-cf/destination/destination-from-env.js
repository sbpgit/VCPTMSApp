...
function getDestinationFromEnvByName(name) {
    ...
    return proxy_util_1.proxyStrategy(destination) === proxy_util_1.ProxyStrategy.INTERNET_PROXY ||
        proxy_util_1.proxyStrategy(destination) === proxy_util_1.ProxyStrategy.PRIVATELINK_PROXY
        ...
        try {
            ...
            if (destination) {
                if (destination.forwardAuthToken) {
                    ...
                    logger.info("Successfully retrieved destination '" + name + "' from environment variable.");
                }
                else {
                    logger.warn("Successfully retrieved destination '" + name + "' from environment variable." +
...
function destinationAuthToken(token) {
    if (token) {
        var decoded = jwt_1.decodeJwt(token);
