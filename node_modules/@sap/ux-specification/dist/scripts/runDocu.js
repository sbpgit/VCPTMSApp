...
const tooltipDiv = d3.select('body').append('div').style('opacity', 1e-6).attr('class', 'tooltip');
...
const zoomListener = d3.zoom().scaleExtent([0.3, 3]).on('zoom', zoom);
...
function filterSearchItems(searchTerm) {
    jquery_1.default('#search-result li').each(function () {
        const currentText = jquery_1.default(this).text().toUpperCase();
...
function createSearch(items) {
    ...
    jquery_1.default.each(items, function (i, v) {
        ...
        jquery_1.default('<li/>').attr('data-path', data[1]).text(split(data[0])).appendTo(searchResult);
    });
...
d3.select('#floorPlan-icon').attr('y', -7).attr('x', -15);
