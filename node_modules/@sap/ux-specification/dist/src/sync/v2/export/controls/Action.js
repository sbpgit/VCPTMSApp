 ...
 */
exports.buildIntentBasedActionId = (baseId, _idBreadcrumbs, breadcrumbs) => {
    const { semanticObjectId, actionId } = common_1.getActionBreadcrumbParts(breadcrumbs);
    return `${baseId}action::${semanticObjectId}::${actionId}`;
};
/**
...
const getFooterActionButtonId = (baseId, _idBreadcrumbs, breadcrumbs) => {
    const { actionId } = common_1.getActionBreadcrumbParts(breadcrumbs);
    return `${baseId}action::${actionId.replace(':2f', '::')}::Determining`;
};
