...
const VIEW_TEMPLATE_TAGTYPE_MAP = new Map([
    [types_1.ViewTemplateType.ResponsiveTableColumnsExtension, { namespace: 'sap.m', name: 'Column' }],
    [types_1.ViewTemplateType.TreeTableColumnsExtension, { namespace: exports.SAP_UI_TABLE, name: 'Column' }],
    [types_1.ViewTemplateType.GridTableColumnsExtension, { namespace: exports.SAP_UI_TABLE, name: 'Column' }],
    [types_1.ViewTemplateType.AnalyticalTableColumnsExtension, { namespace: exports.SAP_UI_TABLE, name: 'AnalyticalColumn' }]
]);
 ...
 * @param definition - fragment content
 * @param viewTemplateType - name of the view template that shall be applied
 * @returns {unknown | undefined} Root column tag name.
 */
function getTagNameFromFragmentJSON(definition, viewTemplateType) {
    const namespaces = getFragmentNamespaces(definition, viewTemplateType ? true : false);
    if (viewTemplateType) {
...
function handleFragmentColumn(columnsInFragment, configColumns) {
    ...
    if (!columnsInFragment['Text'] && !columnsInFragment['Label'] && ((_a = configColumns[columnIndex]) === null || _a === void 0 ? void 0 : _a.text)) {
        columnsInFragment['Text'] = {
            _attributes: {}
        };
    }
    if (columnsInFragment['Text'] && columnsInFragment['Text']['_attributes']) {
        columnsInFragment['Text']._attributes['text'] = (_b = configColumns[columnIndex]) === null || _b === void 0 ? void 0 : _b.text;
    }
    else if (columnsInFragment['Label'] && columnsInFragment['Label']['_attributes']) {
        columnsInFragment['Label']._attributes['text'] = (_c = configColumns[columnIndex]) === null || _c === void 0 ? void 0 : _c.text;
    }
...
function deleteColumnsFromFragment(manifestSection, fragments, pattern, appId) {
    ...
    const fragmentName = fragmentParts[fragmentParts.length - 1] + types_1.FRAGMENTNAMEPART + fragmentType;
...
function exportCustomColumnsToManifest(manifestPath, exportResults, config, appId, logger) {
    ...
    for (const configElement of config) {
        ...
        lsFilePath =
            ((_a = configElement.fragmentName) === null || _a === void 0 ? void 0 : _a.replace(appId, '<root>/webapp').replace(/[.]/g, path_1.sep)) +
...
function insertNewColumnInFragment(column, viewTemplateType, columnsInFragmentDefinition) {
...
function insertNewColumnsInFragment(configColumns, fragmentId, columnsInFragmentDefinition, viewTemplateType) {
    for (const column of configColumns) {
        if (column.fragmentName.endsWith(fragmentId)) {
            columnsInFragmentDefinition = insertNewColumnInFragment(column, viewTemplateType, columnsInFragmentDefinition);
        }
    }
    return columnsInFragmentDefinition;
}
    ...
    for (const fragment of fragments) {
        if (fragment['dataSourceUri'].endsWith(fragmentName)) {
            ...
            if (!fragment.fileContent || fragment.fileContent.length === 0) {
                ...
                columnsInFragmentDefinition = insertNewColumnsInFragment(configColumns, fragmentId, columnsInFragmentDefinition, viewTemplateType);
